...
impl Processor {
    ...
    fn process_init_escrow(accounts: &[AccountInfo],
                           amount: u64,
                           program_id: &Pubkey,
    ) -> ProgramResult {
        ...
        let initializer = next_account_info(account_info_tier)?;
        ...
        let temp_token_account = next_account_info(account_info_tier)?;

        /// 2. `[]` The initializer's token account for the token they will receive should the trade go through
        let token_to_receive_account = next_account_info(account_info_tier)?;
        if *token_to_receive_account.owner != spl_token::id() {
            return Err(ProgramError::IncorrectProgramId);
        }

        /// 3. `[writable]` The escrow account, it will hold all necessary info about the trade.
        let escrow_account = next_account_info(account_info_tier)?;

        /// 4. `[]` The rent sysvar
        let rent = &Rent::from_account_info(next_account_info(account_info_tier)?)?;
        ...
        let (pda_account, _bump_seed) = Pubkey::find_program_address(&[b"escrow"], program_id);


        /// 5. `[]` The token program
        let token_program = next_account_info(account_info_tier)?;
